...
public class JFreeChart implements Drawable, TitleChangeListener,
    ...
    public BufferedImage createBufferedImage(int imageWidth,
                                             int imageHeight,
                                             CreateBufferedImageParameter parameterObject,
                                             ChartRenderingInfo info) {

        info(imageWidth, imageHeight, parameterObject, info);
		BufferedImage image = new BufferedImage(imageWidth, imageHeight,
                BufferedImage.TYPE_INT_ARGB);
        Graphics2D g2 = image.createGraphics();
        AffineTransform st = AffineTransform.getScaleInstance(calScaleX(imageWidth, parameterObject), calScaleY(imageHeight, parameterObject));
        g2.transform(st);
        g2.dispose();
        return image;
    ...
    public double calScaleX(int imageWidth, CreateBufferedImageParameter parameterObject){
    	return imageWidth / parameterObject.getDrawWidth();
    	
    }
    
    public double calScaleY(int imageHeight, CreateBufferedImageParameter parameterObject){
    	return imageHeight / parameterObject.getDrawWidth();
    	
    }

	private void info(int imageWidth, int imageHeight, CreateBufferedImageParameter parameterObject,
			ChartRenderingInfo info) {
		BufferedImage image = new BufferedImage(imageWidth, imageHeight, BufferedImage.TYPE_INT_ARGB);
		Graphics2D g2 = image.createGraphics();
		draw(g2, new Rectangle2D.Double(0, 0, parameterObject.getDrawWidth(), parameterObject.getDrawHeight()), null,
				info);
	}
